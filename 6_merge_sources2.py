...
df = pd.read_csv(
    os.path.join(PATH, "3_preprocessed2.csv"), index_col=[0, 1], parse_dates=["Date"]
)
...
# Standarize on a rolling window: Fundamental data
def zscore(
    x, window=10
):  #  TODO Add suffix _zscore to the names of the standarized columns
    r = x.rolling(window=window, min_periods=1)
    m = r.mean()
    s = r.std(ddof=0)
    masks = s == 0
    z = (
        (x - m).div(s).where(~masks, 0)
    )  # Unintendly sets the first row to 0 instead of Nan
    return z


df_fndmt.loc[:, df_fndmt.columns != "Period_beginning"] = (
    df_fndmt.loc[:, df_fndmt.columns != "Period_beginning"]
    .groupby("Stock")
    .apply(zscore)
)
# Remove the first row for each stock
df_fndmt = df_fndmt.groupby(by="Stock").apply(
    lambda df_fndmt: df_fndmt.droplevel(0).iloc[1:]
)
# Standarize on a rolling window: Ohlcv and mixed data
df_cols_to_standarize = [
    "MarketCap",
    "EnterpriseValue",
    "EVbyRevenue",
    "EVbyEBIT",
...
df.loc[:, df_cols_to_standarize] = (
    df.loc[:, df_cols_to_standarize].groupby("Stock").apply(zscore)
)
# Remove the first row for each stock
df = df.groupby(by="Stock").apply(lambda df: df.droplevel(0).iloc[1:, :])
# Merge both sources on the ohlcv data index
print(
    "Nans in fundamental data before the second merge: ", df_fndmt.isnull().sum().sum()
)
print(
    "Nans in the initially merged data before the second merge: ",
    df.isnull().sum().sum(),
)
df = (
    pd.merge_asof(
        df[df.columns.difference(df_fndmt.columns)].reset_index().sort_values(["Date"]),
