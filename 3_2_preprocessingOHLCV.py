...
df.loc[:, "Returns"] = (
    df["Adjusted_close"]
    .groupby(by="Stock")
    .apply(lambda stock_df: stock_df.pct_change(1))
)
...
for col in new_columns:
    ...
    if results.success == False:
        print(
            f' Results of checking for Nans for the "{col}" feature engineered: ',
            results,
        )
        raise ValueError(
            "Some of the technical indicators engineered have too many null values: "
            + str(col)
        )
    if df.loc[:, col].isin([np.inf, -np.inf]).sum().sum() > 0:
        raise ValueError(
            "Some of the technical indicators have inf values: " + str(col)
        )
