...
#Find optimal hyperparameters
    # Define tunable callbacks
class MyTuner(kt.BayesianOptimization):
  def run_trial(self, *args, **kwargs):
    patience = hp.Int('patience', 0, 10, default=5)
    early_stopping = tf.keras.callbacks.EarlyStopping(patience=patience) 
    log_dir = "TensorBoard_logs/data/" + datetime.datetime.now().strftime("%H%M%S-%Y%m%d")
    tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=log_dir, histogram_freq=1)    
    reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(factor=0.8,patience=patience, min_lr=0.000001, verbose=0, monitor='val_acurracy') 
    callbacks=[
        reduce_lr,
        early_stopping,
...
early_stopping = tf.keras.callbacks.EarlyStopping(patience=patience) 
reduce_lr = tf.keras.callbacks.ReduceLROnPlateau( factor=0.7, patience=patience, min_lr=0.000001, verbose=0)
    # Fit
