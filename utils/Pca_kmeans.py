...
def pca(df, variance,  transpose = False, pcs_vs = True, expl_var = True, loadings = False, loadings_vs = False, row_referents = [], loading_referents = []):
...
#                             If 'transpose = True' then it would be the indexes of rows before transposing the dataframe.
    ...
    loadings_data = pd.DataFrame(pca.components_.T , index=df.columns , columns=['PC'+str(i) for i in range( 1, len( pca.components_ )+1 ) ]  )  
    # Plots
    col_names = df.columns.get_level_values(0)  # get_level_values(0) so it works on multiindex dataframes
    if pcs_vs == True :
        plot_pcs_vs( pc_data, variance, df.index, referents = row_referents, cl_labels = None)                
    if expl_var == True:
        plot_explained_variance( pca)   
    if loadings == True:
        plot_loadings( loadings_data, col_names, loading_referents)
    if loadings_vs == True:
        # The first answer in this link could be useful to display all the subplots in the same figure in a better way:
        # https://stackoverflow.com/questions/52723910/matplotlib-user-changes-amount-of-subplots?noredirect=1&lq=1  
        plot_loadings_vs( loadings = loadings_data, pc1=1, pc2 = 2, subplot = 231, og_columns = col_names,  loading_referents = loading_referents, cl_labels = None)
        plot_loadings_vs( loadings_data, 1, 3, 232, col_names,  loading_referents)
        plot_loadings_vs( loadings_data, 1, 4, 233, col_names,  loading_referents)
        plot_loadings_vs( loadings_data, 2, 3, 234, col_names,  loading_referents)
        plot_loadings_vs( loadings_data, 2, 4, 235, col_names,  loading_referents) 
        plot_loadings_vs( loadings_data, 3, 4, 236, col_names,  loading_referents) 
    plt.show()
    return pc_data, loadings_data, pca
def pca_kmeans(df, variance, n_cl, max_cl, seed, transpose = False, pcs_vs=False, expl_var=False, row_referents = []): 
...
#        Other arguments same as above
    ...
    clusters = {i:df.index[np.where(cl_labels==i)[0]].tolist() for i in np.unique(cl_labels)} #cluster members #cambié columns por index
    #Plots
    if pcs_vs==True :
        plot_pcs_vs( pc_data, variance, df.index, referents = row_referents, cl_labels = cl_labels, title = 'Kmeans clusters by Pca components. On the graph only 1st and 2nd pc.')   
...
def plot_pcs_vs( pc_data, variance, rows, referents = [], cl_labels = None, title = '1st and 2nd principal components'):
...
#       'variance' and 'title' are text boxes on the matplotlib figure
    ...
    plt.text(0.1, 0.93,str(len(pc_data[0,:]))+' principal components', transform = ax.transAxes)
    ...
    if (len(pc_data[0,:]) > 1):          
        ...
        for i, txt in enumerate(rows):
            if txt in referents :
                plt.annotate(txt, (pc_data[i,0], pc_data[i,1]-0.1), fontsize=30, c = colors[i])
                plt.scatter( pc_data[i,0], pc_data[i,1], s=250, c = colors[i], alpha=0.5)
            else:
                plt.annotate(txt, (pc_data[i,0]+0.15, pc_data[i,1]), fontsize=12, alpha=0.5)
    ...
    # If there is just one pca component it plots that one
        ...
        for i, txt in enumerate(rows):
            if txt in referents:
                plt.annotate(txt, (pc_data[i,0], 1), fontsize=25, c = colors[i], ha='left', rotation=-45, rotation_mode='anchor')
                plt.scatter( pc_data[i,0], 1, s = 250, c = colors[i], alpha = 0.5)
            else:
                plt.annotate(txt, (pc_data[i,0], 1), fontsize=12, alpha=0.5, ha='left', rotation=-45, rotation_mode='anchor')
                plt.scatter(pc_data[i ,0], 1 , s = 50, c = colors[i])
def plot_explained_variance( pca ):
# Plot fraction of total variance explained by each principal component
    fig = plt.figure('Explained variance by amount of principal components') # plt.axes( [.65, .6, .2, .2] )   # Swap these lines to plot in the main plot
    plt.title('Explained variance amount by each principal component \n Enough PCs for +{:.2f} explaine variance'.format( pca.explained_variance_ratio_.sum()//0.01/100 ), fontsize = 10)
...
def plot_loadings_vs( loadings, pc1, pc2, subplot, og_columns, loading_referents =  [], cl_labels = None):
# For 2 given principal components, it plots the loadings of one pc 
# (how much the principal component takes from each original feature) vs the loadings of the another
#       'pc1', 'pc2' : Ints. Principal components for which to plot the loadings. i.e. 1 for the 1st, 4 for the 4th.
#       'subplot' : Matplotlib argument for 'pyplot.subplot' to choose where to plot in the subplots grid. i.e. 236 for bottom right on a 2*3 grid
#       'og_columns' : List of strings. Original features reduced by pca.
#       'loading_referents' : List of strings. Original columns whose points are to be ploted in bigger size; and also a different color if no cluster labels are provided.
    plt.figure( 'Vs Loadings', figsize= (20,20) )
    axs = plt.subplot( subplot )
    if cl_labels is None:
        colors = ['b' if x not in loading_referents else 'g' for x in og_columns ]
    else:
        ncl = len( np.unique(cl_labels) )
        plt.text(0.1, 0.9, str(ncl)+' clusters',transform = axs.transAxes)
        colors = map_number_to_color(cl_labels)
    plt.xlabel('PC{} loadings'.format(pc1))
    plt.ylabel('PC{} loadings'.format(pc2))
    for i, txt in enumerate(og_columns):     
        if txt in loading_referents : 
            plt.annotate(txt, (loadings['PC{}'.format(pc1)][i], loadings['PC{}'.format(pc2)][i]-0.01), fontsize=25, c = colors[i] )
            plt.scatter( loadings['PC{}'.format(pc1)][i], loadings['PC{}'.format(pc2)][i], s=250, c = colors[i], alpha = 0.5 )
        else:
            plt.annotate(txt, (loadings['PC{}'.format(pc1)][i]+0.01, loadings['PC{}'.format(pc2)][i]), fontsize=12, alpha=0.5)
            plt.scatter( loadings['PC{}'.format(pc1)][i], loadings['PC{}'.format(pc2)][i], s = 50, c = colors[i] )
...
# Transforms list of numbers to hex codes for colors
    ...
    color = [clrs.rgb2hex(x[:3]) for x in rgb] # As per stack overflow commenter: [cmap] "will return rgba, we take only first 3 so we get rgb"
    return color
