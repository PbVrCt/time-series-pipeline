...
def getVol( close, span0=100, returns_timeframe=pd.Timedelta(days=1) ):
    ...
    df0=df0[df0>0] - 1
    df0=pd.Series(close.index[ df0 ] , index=close.index[close.shape[0]-df0.shape[0]:]) # Easy to inspect for debugging
...
def applyPtSlOnT1(close, events, PtSl): # molecule
    ...
    events_ = events # For multiprocessing with old version of a multiprocessing library
    ...
    for loc, vtc in events_["vtc"].fillna( close.index[-1] ).iteritems(): # If there is no vertical barrier, it takes the last index
        df0 = close.loc[loc:vtc]  
        df0 = (df0 / close[loc] - 1)* events_.at[loc, "side"] # Returns path # The multiplying factor is -1 in case of shorting
...
def getEvents(close, trgt, PtSl=[1,1], minRet=0, vtc=False, vertical_length=pd.Timedelta(days=5), tindex=None, side=None): #numThreads
    """
    ...
    trgt = trgt[trgt > minRet]  # Discard days whose horizontal barriers would have a small width
    trgt.columns = ["trgt"]
    ...
    if side is None:
        ...
        PtSl_ = [PtSl[0], PtSl[1]] # Assymetric barriers in case we have already bet ( side is not None )
    ...
    if vtc is False: 
        ...
        vtc=pd.Series(close.index[vtc],index=close.index[:vtc.shape[0]]) # NaNs at end
    # 4) Use applyPtSlOnT1
    events = pd.concat({'vtc': vtc, 'trgt': trgt, 'side': side_}, axis=1).dropna(subset=['trgt'])
    ...
    events["t1"] = df0.dropna(how="all",axis=0).min(axis=1, skipna=True,numeric_only=False)
...
if __name__ == '__main__' : 
    ...
    """ ), sep="\\s+", index_col=[0]   )
    events = getEvents( df['Adjusted_close'], getVol(df['Adjusted_close'],span0=100), [1,1] )
    labels = getLabels( getEvents( df['Adjusted_close'], getVol(df['Adjusted_close'],span0=100), [1,1] ), df['Adjusted_close'] )
    print( events )
    print( labels )
    ...
    # import os
    # import pathlib
    # file='preprocessed2.csv'
    # path = pathlib.Path(__file__).resolve().parent / 'processed data'
    # df = pd.read_csv( os.path.join( path , file), index_col=[0,1,2])
    # df = df.droplevel( axis=0, level=0 )
    # events = df.groupby(level='Stock').apply( lambda df: getEvents( PtSl=[1,1], close=df['Adjusted_close'].droplevel('Stock'), trgt=getVol(df['Adjusted_close'].droplevel('Stock'), span0=10) ) )
    # labels = df.groupby(level='Stock').apply( lambda df: getLabels( getEvents( PtSl=[1,1],  close=df['Adjusted_close'].droplevel('Stock'),  trgt=getVol(df['Adjusted_close'].droplevel('Stock'), span0=10) ), df['Adjusted_close'].droplevel('Stock') ) )
    # print( events )
    # print( labels )
