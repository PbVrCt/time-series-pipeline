...
from utils.make_labels import getVol, getEvents, getLabels # Slow. TODO getEvents would be sped up by multiprocessing
from utils.sample_against_redundancy import getIndMatrix, seqBootstrap # Very slow. If possible might benefit from multiprocessing/multithreading

np.seterr(divide='raise')

path = pathlib.Path(__file__).resolve().parent / '1_processed data'
df = pd.read_csv( os.path.join( path , '2_merged.csv'), index_col=[0,1], parse_dates=['Date'])

# Feature engineering
og_columns = df.columns
df.loc[:,'MarketCap']= df['Adjusted_close'].astype('float').multiply( df['commonStockSharesOutstanding'].astype('float'))
df.loc[:,'EnterpriseValue'] = df['MarketCap'] + df['Debt'] - df['totalCurrentAssets'].astype('float') # Market Cap + Debt - CashAndCashEquivalents
df.loc[:,'EVbyRevenue'] = np.divide( df['EnterpriseValue'] , df['totalRevenue'].astype('float') )
df.loc[:,'EVbyEBIT'] = np.divide( df['EnterpriseValue'] , df['ebit'].astype('float') )
df.loc[:,'PriceToEarnings']= np.divide( df['MarketCap'] , df['netIncomeApplicableToCommonShares'].astype('float') )     # price / eps =  price /  ( (netIncome - preferred dividends) / Shares Outstanding ) = market cap / netIncomeApplicableToCommonShares
df.loc[:,'PriceByFreeCF'] = np.divide( df['Adjusted_close'] , ( df['totalCashFromOperatingActivities'].astype('float') - df['capitalExpenditures'].astype('float') ) )# Price / Free Cash Flow
df.loc[:,'PriceToBookRatio'] = np.divide( df['MarketCap'] , (df['totalAssets'].astype('float') - df['totalLiab'].astype('float') ) ) # Price / ( BookValue / Shares Outstanding ) = Market Cap / BookValue 
...
# TODO More data validation on the engineered features, and also on the labels if needed
new_columns = set(df.columns) - set(og_columns)
for col in new_columns:
    if df.loc[:,col].isin([np.inf, -np.inf]).sum().sum() > 0:
        df.loc[:,col] = df[col].replace([np.inf, -np.inf], np.nan).groupby(by='Stock').ffill()
        warnings.warn('One of the engineered features had inf values: '+str(col)+'. Infs replaced for nans and ffilled.')
